...
class DatabaseService
  ...
  Future<void> addShelf(Shelf shelf) async {
    await database.insert('shelves', {'name': shelf.name},conflictAlgorithm:ConflictAlgorithm.ignore );
  }

  @override
  Future<List<Shelf>> getShelves()  async{
    final shelvesMap =  await database.query('shelves');
    final shelvesList = shelvesMap.map((element) =>  Shelf.fromJson(element)).toList();
    return shelvesList;
  }
